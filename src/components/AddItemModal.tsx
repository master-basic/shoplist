...
export function AddItemModal({ isOpen, onClose, onSubmit, existingItems = [], assignees = [] }: AddItemModalProps) {
  ...
  return (
    <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4" role="dialog" aria-modal="true" aria-labelledby="add-item-title">
      <div className="bg-white rounded-2xl shadow-xl max-w-lg w-full max-h-[90vh] overflow-y-auto">
        {/* Header */}
        <div className="flex items-center justify-between p-6 border-b border-slate-100">
          <h2 id="add-item-title" className="text-xl font-semibold text-slate-900">Add New Item</h2>
          <button
            onClick={onClose}
            aria-label="Close"

...
// =====================================================
// PURCHASE SESSION ROUTES
// =====================================================

app.post('/api/purchase-sessions', async (req, res) => {
  try {
    const { listId, storeName, userId, householdId, items } = req.body;
    if (!listId || !userId || !items || !items.length) {
      return res.status(400).json({ error: 'listId, userId, and items array are required' });
    }
    const totalAmount = items.reduce((sum, i) => sum + (i.totalPrice || i.unitPrice || 0), 0);
    const receiptResult = await pool.query(
      `INSERT INTO receipts (household_id, list_id, user_id, name, total_amount, currency, status)
       VALUES ($1, $2, $3, $4, $5, $6, $7) RETURNING *`,
    ...
    for (const item of items) {
      ...
      await pool.query(
        `INSERT INTO price_history (item_name, store_name, unit_price, quantity, session_id, bought_by, purchased_at)
         VALUES ($1, $2, $3, $4, $5, $6, $7)`,
        [item.name || item.itemName, storeName || null, item.unitPrice || 0, item.quantity || 1, receipt.id, userId, new Date().toISOString()]
      );
    }
...
app.get('/api/purchase-sessions/user/:userId', async (req, res) => {
  try {
    const { userId } = req.params;
    const result = await pool.query(
      `SELECT r.*, u.name as user_name FROM receipts r
       LEFT JOIN users u ON r.user_id = u.id
       WHERE r.user_id = $1 AND r.status = 'purchased'
       ORDER BY r.created_at DESC`,
      [userId]
    );
    const sessions = [];
    for (const row of result.rows) {
      const itemsResult = await pool.query('SELECT * FROM receipt_items WHERE receipt_id = $1 ORDER BY created_at', [row.id]);

...
function extractProductsFromHtml(htmlPath) {
  ...
  try {
    ...
    // Look for venue-assortment data in queries
    ...
    if (!venueAssortment) {
      return { error: 'venue-assortment not found', source: htmlPath };
    }

    // Extract products from venue assortment
    const products = [];

    // Venue assortments are typically in venue-assortment data
